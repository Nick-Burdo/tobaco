			...
			jQuery(".picture-slides-container").each(function () {
				var elm = jQuery(this),
					...
					setImage = function () {
						...
						var ancorElement = elm.find('.picture-slides-thumbnails a').eq(imageIndex); 
						var imageElement = elm.find('.picture-slides-thumbnails img').eq(imageIndex);
						var href = elm.find('.picture-slides-link').eq(imageIndex).text(); 
						
						var imageItem = images[imageIndex];

						mainImage.attr({
							src : ancorElement.attr('href'),
							alt : imageElement.attr('alt')
						});
						
						mainImage.attr("title", imageElement.attr('title'));
						
					
						// Set image text
						if (imageTextContainer.length > 0) {
							imageTextContainer.html(imageElement.attr('title'));
						}
					
						// Set image link
						imageLink.attr("href", href);
//						elm.find('.picture-slides-text-carier').attr("href", href);
